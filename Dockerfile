...
RUN { \
    ...
    echo '    end=";"'; \
    echo '  fi'; \
    echo '  sed -ri -e "s/($start\s*).*($end)$/\1$(sed_escape_rhs "$(php_escape "$value" "$var_type")")\3/" wp-config.php'; \
    echo '}'; \
    ...
    echo '  currentValue="$(sed -rn -e "s/define\((([\'\''\"])$uniqueKey\2\s*,\s*)(['\''\"])(.*)\3\);/\4/p" wp-config.php)"'; \
